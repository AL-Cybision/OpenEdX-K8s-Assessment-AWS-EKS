...
TF_DIR="${TF_DIR:-${SCRIPT_DIR}/../configs/terraform/data-layer}"
command -v terraform >/dev/null 2>&1 || { echo "terraform not found in PATH" >&2; exit 1; }

usage() {
  cat <<'EOF'
Usage:
  scripts/61-backup-restore.sh rds
  ...
  scripts/61-backup-restore.sh ebs-volume <SNAPSHOT_ID> <AZ>
...
case "${cmd}" in
  rds)
    ...
    if [ -z "${SNAPSHOT_ID}" ] || [ "${SNAPSHOT_ID}" = "None" ]; then
      echo "No manual snapshots found for ${DB_ID}. Run scripts/60-backup-run.sh or set SNAPSHOT_ID." >&2
      exit 3
    fi

...
RUN_APP_INIT="${RUN_APP_INIT:-true}"
...
cp -f "${REPO_ROOT}/configs/tutor/plugins/openedx-mfe-https.py" "${TUTOR_PLUGINS_DIR}/openedx-mfe-https.py"
cp -f "${REPO_ROOT}/configs/tutor/plugins/openedx-elasticsearch.py" "${TUTOR_PLUGINS_DIR}/openedx-elasticsearch.py"
...
kubectl kustomize "${TUTOR_ENV_DIR}" | \
  "${TUTOR_BIN%/tutor}/python3" "${REPO_ROOT}/configs/k8s/tutor/postrender-remove-caddy.py" | \
...
if [[ "${RUN_APP_INIT}" == "true" ]]; then
  "${REPO_ROOT}/scripts/40-openedx-init.sh"
fi
